...
class SSCLAdapted(nn.Module):
    def __init__(self, numberWords, numberFilters=128, filterSize=5, embeddingSize=300):
        ...
        self.lstm = nn.LSTM(self.number_words, self.number_filters // 2, batch_first=True, proj_size=self.number_filters // 4)

        self.dense = nn.Linear(self.number_filters * numberFilters // 4, 1)
    ...
    def forward(self, x):
        ...
        xLSTM = self.lstm(xConvReshape)[0]
        xLSTMFinal = torch.flatten(xLSTM.squeeze(), start_dim=1)
        xDense = self.dense(xLSTMFinal)
        xSoftmaxed = self.softmax(xDense.squeeze())
        #print(xDense.shape)
        return xSoftmaxed
